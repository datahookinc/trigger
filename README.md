...
`insertRows(r: Omit<T, '_pk'>[], batch?: boolean): T[]`: insert multiple rows into the table. The user does not need to provide the _\_pk_ property for each row as this will be handled automatically. This will return the newly inserted rows. Any attached triggers (e.g., **beforeInsertTrigger**) will be fired for each row. By default, components with a matching `use()` hook will only be rerendered all rows are inserted (batch). To override this behaviour, pass `false` as the second argument, instructing trigger to rerenders components on each insert, which is the same behaviour as manually calling `insertRow()` multiple times.

`onBeforeInsert(fn: (v: T) => T | void | boolean): void`: a trigger function that can be attached to the table. The function will receive the row being inserted and can make changes to it. The user can cancel the insert by returning `false`. Returning nothing or `true` will ignore any changes made in the function and insert the row as originally intended.
