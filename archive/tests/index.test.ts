...
import type { TriggerAPI, TriggerQueue, TriggerQueueItem } from '../src/index';
import CreateStore, { NewTriggerQueue, CreateUtils } from '../src/index';
...
export const utils = CreateUtils<MyUtils>(store); // TODO: it is trying to protect me from the mess I already know exists
...
describe('Testing store', () => {
    ...
    it('should find all table rows when passing a function', () => {
        const { result } = renderHook(() =>
            findTableRows<Customer>(utils.tables.customers, (v: Customer) => {
    ...
    it('should find the first table row when passing a function', () => {
        const { result } = renderHook(() =>
            findTableRow<Customer>(utils.tables.customers, (v: Customer) => {
                return v.firstName === 'Billy' || v.firstName === 'Tammy';
            }),
        );
        // Note: it is unwise to rely on the rows being returned in a particular order
