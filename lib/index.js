...
export function CreateTable(t) {
    ...
    const table = Object.assign(Object.assign({}, t), { _id: initialAUTOID }); // put AUTOID last to override it if the user passes it in erroneously; manually add the "_id" so the user does not need to
    ...
    const columnNames = Object.keys(table); // the user provided column names + "_id" (this type is technically wrong - it does not properly add the _id; it should be keyof TableRow<T>)
    ...
    const _getAllRows = () => {
        ...
        for (let i = 0, numValues = table['_id'].length; i < numValues; i++) {
            ...
            for (let j = 0, numArrays = columnNames.length; j < numArrays; j++) {
                entry[columnNames[j]] = deepClone(table[columnNames[j]][i]);
            }
            entries.push(entry);
        }
        return entries;
    ...
    const _insertRow = (newRow) => {
        ...
        let entry = Object.assign(Object.assign({}, newRow), { _id: newAUTOID });
        ...
        for (const k of Object.keys(entry)) {
            table[k].push(deepClone(entry[k]));
        }
    ...
    const _updateRow = (idx, cv, nv) => {
        ...
        for (const k of Object.keys(updateValue)) {
            if (table[k] !== undefined && k !== '_id') {
                const v = updateValue[k];
                if (v !== undefined) {
                    table[k][idx] = deepClone(v);
                }
            }
        }
    ...
    const _getRowIndexById = (id) => {
        return table._id.findIndex((v) => v === id);
    };
    return {
        use(where = null, notify = []) {
            const [v, setV] = useState(() => _getRows(where)); // initial value is set once registered to avoid race condition between call to useState and call to useEffect
                    ...
                    if (whereClause.current) {
                        ...
                        if (typeof whereClause.current === 'function') {
                            ...
                            if (keys.length === 0) {
                                ...
                                for (const k of keys) {
                                    if (!columnNames.includes(k)) {
                                        logWarning(`column '${String(k)}' in where claused does not exist in table; use() will not return any rows until this is fixed`);
                                        return [];
                                    }
                                }
        ...
        useOne(where = null, notify = []) {
            const [v, setV] = useState(() => this.findOne(where || undefined)); // initial value is set once registered to avoid race condition between call to useState and call to useEffect
            // NOTE: this is required to avoid exhaustive-deps warning, and to avoid calling useEffect everytime v changes
            const hasChanged = useRef((newValue) => rowHasChanged(v, newValue));
            const notifyList = useRef(Array.from(new Set(notify)));
            const whereClause = useRef(where);
            hasChanged.current = (newValue) => rowHasChanged(v, newValue);
            useEffect(() => {
        ...
        updateById(AUTOID, setValue, render = true) {
            let idx = -1;
        ...
        updateOne(where, setValue, render = true) {
        ...
        updateMany(setValue, where, options) {
            const ops = Object.assign({ batchNotify: true, render: true }, options);
        ...
        count(where) {
            switch (typeof where) {
                case 'object': {
                    // make sure the requested columns exist in the table; if they don't all exist, return undefined
                    const keys = Object.keys(where); // needed to avoid flipping between string and keyof T
                    for (const k of keys) {
                        if (!columnNames.includes(k)) {
                            return 0;
                        }
        ...
        toJSON(index = false, where, n = 50) {
            ...
            if (index === false) {
                ...
                returnedRows = rows.reduce((acc, _a) => {
                    var { _id: _ignore } = _a, x = __rest(_a, ["_id"]);
                    acc.push(x);
                    return acc;
                }, []);
            }
